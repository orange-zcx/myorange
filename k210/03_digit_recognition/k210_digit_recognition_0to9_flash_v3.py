...
while True:
    ...
    if locked_rect and candidate:
        ...
        # A true 0 or 8 cannot have the very narrow shape of digit 1.
        shape_ratio = locked_rect[2] * 100 // locked_rect[3]
        shape_forces_one = (
            shape_ratio <= 48 and
            (model_digit == 0 or model_digit == 1 or model_digit == 8)
        )
        ...
        if shape_forces_one:
            ...
            final_digit = model_digit
            reliable = confidence >= MIN_CONFIDENCE
            method = "MODEL"

        if reliable:
            if final_digit == last_prediction:
                same_prediction_count += 1
            else:
                last_prediction = final_digit
                same_prediction_count = 1
        else:
            last_prediction = -1
            same_prediction_count = 0

        print("MODEL:%d CONF:%d%% FINAL:%d METHOD:%s RATIO:%d%% SAME:%d" % (
            model_digit, confidence_percent, final_digit,
            method, shape_ratio, same_prediction_count
        ))

        if reliable and same_prediction_count >= REQUIRED_SAME_RESULTS:
            if shape_forces_one:
                result_text = "DIGIT:1 SHAPE"
                print("DIGIT: 1  METHOD: SHAPE  MODEL:%d  RATIO:%d%%" % (
                    model_digit, shape_ratio
                ))
            else:
                result_text = "DIGIT:%d %d%%" % (
                    final_digit, confidence_percent
                )
            ...
            if shape_forces_one:
                result_text = "GUESS:1 SHAPE"
            else:
                result_text = "GUESS:%d %d%%" % (
                    final_digit, confidence_percent
                )
    ...
    img.draw_string(4, 4, result_text, color=255, scale=2)
    lcd.display(img)
