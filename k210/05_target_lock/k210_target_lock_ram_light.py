...
# Lightweight specified-target lock.
# No KPU model is loaded. At startup, show the requested target alone.
# The normalized target is learned into RAM, then matched among 3 objects.
...
print("LIGHTWEIGHT TARGET LOCK")
print("No KPU model; QVGA is kept")
    ...
    while True:
        ...
        if chosen_rect:
            ...
            if stable_frames >= LEARN_STABLE_FRAMES:
                ...
                reference.draw_image(normalized, 0, 0)
                ...
                print("STEP 2: add the other digit cards")
                print("==============================")
...
reference = learn_reference()
gc.collect()

print("SEARCH STARTED")
print("The learned target alone may acquire")
...
while True:
    ...
    # detection tuple: similarity, similarity percent, rectangle
    detections = []
    ...
    for item in candidate_items:
        ...
        gc.collect()
        similarity_result = normalized.get_similarity(
            reference
        )
        ...
        detections.append(
            (similarity, similarity_percent, rect)
        )
        del normalized
    ...
    if best_detection:
        search_verified = (
            best_similarity >= MIN_SIMILARITY and
            (
                margin >= MIN_MARGIN or
                best_similarity >= STRONG_SIMILARITY
            )
        )
    ...
    for detection in detections:
        similarity_percent = detection[1]
        rect = detection[2]
    ...
    if target_active:
        ...
        best_percent = int(best_similarity * 100)
