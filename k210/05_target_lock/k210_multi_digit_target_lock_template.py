...
# Multi-digit target lock V2:
# neural network + saved target template + per-object voting.
...
print("Rule: same object needs %d verified frames" % VOTES_TO_ACQUIRE)
...
while True:
    ...
    # detection tuple:
    # model digit, confidence, confidence percent,
    # similarity, similarity percent, rectangle
    detections = []
    ...
    for item in candidate_items:
        ...
        similarity_result = digit_112.get_similarity(target_template)
    ...
    # Find the highest and second-highest template similarity.
    best_detection = None
    best_similarity = -2.0
    ...
    if not target_active:
        if search_verified:
            ...
            if same_spatial_object(
                    current_rect, pending_rect, MAX_PENDING_JUMP):
                pending_votes += 1
            else:
        ...
        # After acquisition, select a verified candidate near the old target.
        ...
        if track_detection:
            ...
            smooth_x = (smooth_x * 3 + center_x(rect)) // 4
            smooth_y = (smooth_y * 3 + center_y(rect)) // 4
    ...
    # Draw candidates after all inference calls.
    for detection in detections:
        model_digit = detection[0]
    ...
    if target_active:
        ...
        print(
            "SEARCH target=%d bestS=%d margin=%d verified=%d votes=%d/%d" %
            (
                TARGET_DIGIT,
                best_sim_percent,
                margin_percent,
        ...
        if pending_rect:
            img.draw_rectangle(pending_rect, color=220, thickness=2)
